...
static Rune rkup = Kup;
static Rune rkdown= Kdown;
...
mksnarf(void)
{
	...
	p = getpid();
	seprint(buf, buf+sizeof(buf), "/tmp/rio.snarf.%d", p);
	return create(buf, ORDWR|ORCLOSE, 0600);
}
...
threadmain(int argc, char *argv[])
{
	...
	snarffd = open("/dev/snarf", ORDWR|OCEXEC);
	if (snarffd < 0)
		snarffd = mksnarf();
	if (snarffd < 0){
		fprint(2, "rio: can't open or create snarf: %r\n");
		exits("snarf open");
	}
	getsnarf();
...
putsnarf(void)
{
	...
	for(i=0; i<nsnarf; i+=n){
		...
		if(n >= 256)
			n = 256;
		if(fprint(snarffd, "%.*S", n, snarf+i) < 0)
			break;
	}
}
