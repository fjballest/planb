...
int
quote(Message *m, Window* w, char *dir, char *quotetext)
{
	char *body, *type;
	int i, n, nlines;
	char **lines;
...
void
mkreply(Message *m, char* subj, char *label, char *to, Plumbattr *attr, char *quotetext)
{
	Message *r;
	char *dir, *t;
	int quotereply;
	...
	r->w = newwindow(r->name);
	wintagwrite(r->w, "Post |q", 5+4);
	r->tagposted = 1;
